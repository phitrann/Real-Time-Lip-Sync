...
from rtlipsync.utils.blending import get_image_blending, get_image_prepare_material, FaceParsing
from rtlipsync.utils.preprocessing import get_landmark_and_bbox, read_imgs
from rtlipsync.utils.utils import datagen, load_all_model


def setup_ffmpeg_env(model_dir):
    """
    Set up the FFMPEG environment by downloading and extracting FFMPEG binaries
    if not already present in the given directory.

    Args:
        model_dir (str): Path to the directory where FFMPEG should be set up.
    """
...
def init_digital_model(model_dir, use_float16=False):
    """
    Initialize digital models by downloading model files from HuggingFace and loading them into memory.

    Args:
        model_dir (str): Directory to store the downloaded models.
        use_float16 (bool): Whether to use float16 precision for the models (default: False).

    Returns:
        Tuple: Loaded models including audio processor, VAE, UNet, and positional encoder (PE).
    """
...
def load_pose_model(model_dir):
    """
    Load the pose model for body landmark detection using the DWPose model.

    Args:
        model_dir (str): Directory to store and load the model.

    Returns:
        pose_model (nn.Module): The initialized pose model.
    """
    ...
    config_file = r"./rtlipsync/utils/dwpose/rtmpose-l_8xb32-270e_coco-ubody-wholebody-384x288.py"
...
def video2imgs(vid_path, save_path, ext=".png", cut_frame=10000000):
    ...
    Args:
        ...
        save_path (str): Path to save the extracted images.
        ext (str): Image file extension (default: ".png").
        cut_frame (int): Maximum number of frames to extract (default: 10000000).
    """
...
class Avatar:
    """
    A class to handle avatar creation, processing, and inference from videos.

    Attributes:
        avatar_id (str): Unique identifier for the avatar.
        video_path (str): Path to the input video.
        bbox_shift (tuple): Bounding box shift values.
        model_dir (str): Directory for storing models.
        work_dir (str): Working directory for avatar processing.
        preparation_force (bool): Whether to force re-preparation of avatar data.
    ...
    def __init__(self, avatar_id, work_dir, model_dir, video_path, bbox_shift, batch_size, fps, preparation_force):
        ...
        self.face_parsing_model = FaceParsing() #load_face_parsing_model(self.model_dir)
    ...
    def change_character(self, avatar_id, video_path=""):
        """
        Change the character's avatar and optionally switch the video source.

        Args:
            avatar_id (str): New avatar ID.
            video_path (str): New video path to switch to (optional).
        """
    ...
    def init(self, vae_model, face_parsing_model):
        """
        Initialize avatar data by preparing the required material and loading frames, coordinates, and latent data.

        Args:
            vae_model: Variational Autoencoder (VAE) model for processing frames.
            face_parsing_model: Model for face parsing and landmark detection.
        """
